...
<center>
<br><br>
<font face=Arial size=4><b>Report anno 2013<br><br>Visualizza e gestisci report</b></font>
<br><br>

<br>
<a href="fatturexls.php?id_annoreport=2013" class="TinyBlack"><img src="img/excel.jpg"> Scarica elenco report</a>
...
echo "<form action=\"visualizza_interventi_2013.php\" method=\"post\"  name=\"form1\" id=\"form1\">";
...
if ($id_tecnico !=45) {
$query = "SELECT MIN(id_interventi) as id_interventi, MIN(data_intervento) as data_intervento, id_proposta,id_cliente,id_tecnico,durata_intervento,sede_intervento,motivo_intervento,status_intervento,chiave_univoca FROM `Interventi` WHERE data_intervento > '2012-12-31' AND data_intervento < '2014-01-01'  AND id_cliente <> 363  AND id_cliente <> 362   AND id_cliente <> 380 AND id_cliente <> 370 AND id_cliente <> 369 AND id_cliente <> 379 AND id_tecnico = $id_tecnico GROUP BY chiave_univoca ORDER BY data_intervento DESC";
}
else {
$query = "SELECT MIN(id_interventi) as id_interventi, MIN(data_intervento) as data_intervento, id_proposta,id_cliente,id_tecnico,durata_intervento,sede_intervento,motivo_intervento,status_intervento,chiave_univoca FROM `Interventi` WHERE data_intervento > '2012-12-31' AND data_intervento < '2014-01-01'  AND id_cliente <> 363  AND id_cliente <> 362   AND id_cliente <> 380 AND id_cliente <> 370 AND id_cliente <> 369 AND id_cliente <> 379 GROUP BY chiave_univoca ORDER BY data_intervento DESC";
}
